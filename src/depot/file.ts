...
import {promisify1, promisify3, allSettled} from "./promiseHelpers";
 ...
 */
function calcDestFile(srcFile: File, dstDirOrFile: Directory | File, dstFileName?: string): File
{
    let destFile: File;
...
export class File
{
     ...
     */
    public copy(dstDirOrFile: Directory | File, dstFileName?: string, options?: IFileCopyOptions): Promise<File>
    {
        const realOptions: IFileCopyOptions = _.defaults(options, {overwrite: false});

        //
        // Based on the parameters, figure out what the destination file path is
        // going to be.
        //
        const destFile: File = calcDestFile(this, dstDirOrFile, dstFileName);

        return allSettled([this.getHash(), destFile.getHash()])
        .then(([srcHashInspection, destHashInspection]) => {
     ...
     */
    public copySync(dstDirOrFile: Directory | File, dstFileName?: string, options?: IFileCopyOptions): File
    {
        const realOptions = _.defaults(options, {overwrite: false});
     ...
     * @return A Promise for a File representing the destination file.
     */
    public move(dstDirOrFile: Directory | File, dstFileName?: string, options?: IFileCopyOptions): Promise<File>
    {
        // Copy the file.
     ...
     * @return A File representing the destination file.
     */
    public moveSync(dstDirOrFile: Directory | File, dstFileName?: string, options?: IFileCopyOptions): File
    {
        //
        // Do the copy.
        //
        const dstFile = this.copySync(dstDirOrFile, dstFileName, options);
